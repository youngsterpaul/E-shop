...
import { Search as SearchIcon, ShoppingCart, User, Menu as MenuIcon, X, Home, Package, LogOut, Settings, MapPin, ShoppingBag, Search, Heart, Truck } from 'lucide-react';
...
const Header = () => {
  ...
  const mainNavItems = [{
    ...
    label: 'Wishlist',
    href: '/wishlist',
    description: 'Check your wishlist products'
  }, {
    label: 'About Us',
    href: '/about',
    description: 'Learn more about SmartKenya'
  }, {
    label: 'Careers',
    href: '/careers',
    description: 'Get your dream job at smartkenya'
  }, {
    label: 'FAQs',
    href: '/faq',
    description: 'Get answers to most asked questions'
  }, {
    label: 'Privacy Policy',
    href: '/privacy',
    description: 'Know your privacy at smartkenya'
  }, {
    label: 'Returns',
    href: '/returns',
    ...
    label: 'T&C',
    href: '/terms',
    description: 'Terms and Conditions of Smartkenya'
  }];
  return <>
      ...
      {!isMobile && <nav className={`p-1 bg-white border-b transition-transform duration-300 ease-in-out ${showBottomNav ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`} role="navigation" aria-label="Main Navigation">
          <div className="max-w-6xl lg:max-w-7xl container mx-0 px-[144px]">
        ...
        <div className={`max-w-6xl lg:max-w-7xl container mx-auto ${isMobile ? 'py-2 px-3 /border-b /border-gray-200' : `px-4 py-4 transition-all duration-300 /fixed top-0 left-0 right-0 /z-50`}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            {!isMobile && !hideMainHeader && <>
              <div className="flex flex-1 mx-4 lg:mx-8 relative">
                <EnhancedSearchInput value={searchQuery} onChange={setSearchQuery} onSearch={handleSearch} placeholder="Search for products, brands, or categories..." className="w-full" />
              </div>
            
            <div className="flex items-center space-x-4">
              <Link to="/cart" aria-label='View Cart' className="relative text-gray-700 hover:text-primary transition-colors">
                <ShoppingCart size={24} />
                {totalItems > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full min-w-[18px] h-[18px] flex items-center justify-center">
                    {totalItems > 99 ? '99+' : totalItems}
                  </span>}
              </Link>

              {isMobile && <Link to="/search">
              ...
              {user ? <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" className="px-2 hidden md:flex ml-auto">
                      <User className="h-6 w-6" /> Account
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    <div className="px-2 py-1.5 text-sm font-medium">
                      {profile?.first_name && profile?.last_name ? <p>{profile.first_name} {profile.last_name}</p> : <p>{user.email}</p>}
                    </div>
                ...
                </DropdownMenu> : <Button variant="ghost" className="px-2" onClick={() => navigate('/auth')}>
                  <User className="h-6 w-6" /> Account
                </Button>}
            </div>  
          </>}
