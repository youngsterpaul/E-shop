...
import { Search, Package, MapPin, Phone, Mail, CheckCircle2, XCircle, Loader2, ShoppingBag } from 'lucide-react';
...
// Utility to get styles & icon for status
const statusStyles = {
...
const getStatusColor = (status: string) => statusStyles[status.toLowerCase()] || statusStyles.default;
...
const getCurrentStepIndex = (status: string) => {
  ...
  return trackingSteps.findIndex((step) => step.status === statusLower);
};

const VariantSelections = ({ variant_selections }: { variant_selections?: Record<string, any> }) => {
  if (!variant_selections || Object.keys(variant_selections).length === 0) return null;
  return (
    <div className="text-sm text-gray-500 mb-2">
      {Object.entries(variant_selections).map(([key, value], idx, arr) => (
        <span key={key}>
          {key}: {value}
          {idx < arr.length - 1 && ', '}
        </span>
...
const OrderStatusTimeline = ({ status }: { status: string }) => {
  const currentStep = useMemo(() => getCurrentStepIndex(status), [status]);
          ...
          return (
            <div key={step.status} className="relative flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                {isCurrent && <p className="text-sm text-primary font-medium mt-1">Current Status</p>}
              </div>
      ...
      <main className={`flex-grow container py-8 px-4 ${!isMobile ? 'xl:px-24' : ''}`}>
        ...
        <Card className="max-w-2xl mx-auto mb-8 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>You can find your order number in your order confirmation email</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleTrackOrder} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90 h-12 text-lg" disabled={loading}>
                {loading ? (
                  <>
            ...
            <Card className="shadow-sm">
              <CardHeader className="border-b">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-2xl">Order #{order.order_id.slice(-8).toUpperCase()}</CardTitle>
                    <p className="text-sm text-gray-500 mt-1">Placed on {format(new Date(order.created_at), 'PPP')}</p>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="divide-y divide-gray-100">
                      {order.items.map((item) => (
                        <div key={item.id} className="flex py-4 items-center first:pt-0 last:pb-0">
                          <div className="flex items-center flex-1">
                            ...
                            <div className="ml-4 flex-1">
                              ...
                              <VariantSelections variant_selections={item.variant_selections} />
