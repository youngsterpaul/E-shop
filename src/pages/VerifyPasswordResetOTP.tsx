...
import { KeyRound, ArrowLeft, RefreshCw, ChevronLeft, ArrowRight } from 'lucide-react';
...
const VerifyPasswordResetOTP = () => {
  ...
  const handleVerify = async (code?: string) => {
    ...
    try {
      ...
      toast({ title: 'Verification successful!', description: 'Please enter your new password' });
      setTimeout(() => navigate('/auth?mode=reset'), 1000);
    } catch (error: any) {
    ...
    <div className="flex justify-center gap-2" onPaste={handlePaste}>
      {otp.map((digit, index) => (
        <Input
          ...
          className={`
            ${size === 'lg' ? 'w-12 h-14 text-xl' : 'w-11 h-13 text-lg'}
            text-center font-bold transition-all duration-200 border-2 rounded-xl
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-primary/5 via-background to-accent/5">
      <div className="w-full max-w-md animate-fade-in">
        <div className="bg-card/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-border/50 p-8">
          ...
          {/* OTP Input */}
          <div className="mb-6">
            <div className="mb-4">{otpCluster('lg')}</div>
